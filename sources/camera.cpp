...
camera::camera(int cameraType, GraphicsPipeline& graphics, Player* player)
{
    switch (cameraType)
    {
	case CameraManager::CameraTypes::Title:
		initialize = &camera::titleInitialize;
		update = &camera::titleUpdate;
		break;
	case CameraManager::CameraTypes::Game:
...
void camera::Update(float elapsedTime, Player* player)
{
	(this->*update)(elapsedTime, player);
	AttitudeControl(elapsedTime);
	DebugGUI();
...
void camera::titleUpdate(float elapsedTime, Player* player)
{
}
...
void camera::gameUpdate(float elapsedTime, Player* player)
{
	using namespace DirectX;

...
SpriteDissolve::SpriteDissolve(ID3D11Device* device, const wchar_t* filename, const wchar_t* mask_filename,
    size_t max_sprites, bool blur)
    : max_vertices(max_sprites * 6)
{
    ...
    // 頂点シェーダーオブジェクトの生成
    const char* cso_name{ "shaders/sprite_dissolve_vs.cso" };
    ...
    // ピクセルシェーダーオブジェクトの生成
    const char* cso_name_ps;
    blur ? cso_name_ps = "shaders/sprite_dissolve_blur_ps.cso" : cso_name_ps = "shaders/sprite_dissolve_ps.cso";
...
void SpriteDissolve::begin(ID3D11DeviceContext* dc)
{
    ...
    dc->PSSetShaderResources(1, 1, mask_shader_resource_view.GetAddressOf());
}

void SpriteDissolve::end(ID3D11DeviceContext* dc)
{
